...
/**
 * generateImportPath takes two paths and generates the path that _from_ would
 * use to import _to_. Boths paths must be use the same frame of reference,
 * such as process.cwd() or both be absolute. If only one is absolute, the other
 * must be relative to process.cwd(). Imports of package names are left as-is.
 *
 * The generated import does not include an extension.
 */
const generateImportPath = (from: string, to: string) => {
  if (!to.startsWith('.') && !to.startsWith(upath.separator)) {
    // This is an absolute import of a package name.
    return to;
  }
  ...
  ret = ret.replace(/\.ts$/, '');
  // Regardless of the OS separator, always use forward slash for ECMAScript.
...
const generateHandlerFile = async ({
  ...
  if (hasZValidator && validatorModule != null) {
    imports.push(
      `import { zValidator } from '${generateImportPath(path, validatorModule)}';`,
    );
  }
  ...
  imports.push(
    `import { ${Object.values(verbs)
      ...
      .join(',\n')} } from '${generateImportPath(path, contextModule)}';`,
  );
  ...
  if (hasZValidator) {
    imports.push(
      getZvalidatorImports(
        Object.values(verbs),
        generateImportPath(path, zodModule),
        validatorModule === '@hono/zod-validator',
      ),
    );
  }
...
const generateContextFile = ({
  ...
  if (imps.size > 0) {
    content += `import type {\n${[...imps]
      .toSorted()
      .join(',\n  ')}\n} from '${generateImportPath(path, schemaModule)}';\n\n`;
  }
  ...
  if (output.override.hono.handlers) {
    ...
    ImportHandlersImplementation = operationNames
      .map((operationName) => {
        ...
        const handlersPath = generateImportPath(
