...
import { ModelStyle, type GeneratorImport, type NormalizedOutputOptions } from '../types';
...
export const generateImportsForBuilder = (
  ...
  imports.forEach((imp) => {
    ...
    if (imp.specKey && (imp.specKey.startsWith('./') || imp.specKey.startsWith('../'))) {
      zodImports.push(imp);
    } else {
  ...
  if (output.modelStyle === ModelStyle.ZOD) {
    ...
    const zodImportsByPath = zodImports.reduce<Record<string, GeneratorImport[]>>((acc, imp) => {
    ...
    const zodImportsResult = Object.entries(zodImportsByPath).map(([path, exps]) => ({
      exports: exps,
  ...
  const regularImportsResult = output.schemas && !output.indexFiles
