...
Pass global options to override config file settings. See the complete [GlobalOptions interface](https://github.com/orval-labs/orval/blob/706b86bdfdb39b3c150a62fbf3c4c74ca12a0c50/packages/core/src/types.ts#L718) for all available options.
...
const globalOptions: GlobalOptions = {
  ...
  biome: true,                   // Format with Biome (alternative to Prettier)
  
  // HTTP client configuration
  client: 'fetch',               // Override HTTP client: 'axios' | 'angular' | 'fetch' | 'swr' | 'react-query' | 'vue-query' | 'svelte-query'
  ...
  tsconfig: {                    // Inline tsconfig (Tsconfig object)
    compilerOptions: {
      target: 'ES2020',
