...
Custom handler function that replaces the default response shaping and error mapping of every generated handler. When set, each handler binds the tool arguments to a `fetcher` and calls your function with it and the tool call context. The return value is used as the tool result as is. Return `structuredContent` only when the response body is a plain object: the generated server declares an `outputSchema` for exactly those tools, and MCP rejects arrays and primitives in `structuredContent`.
...
When `useDatesTransform` is also enabled, it converts only the schema-declared
`format: date`/`date-time` fields instead of every string the reviver sees.
...
A `oneOf`/`anyOf` without an OpenAPI
...
per mapping key. `additionalProperties` maps are converted with a loop over
...
place of `properties` drops both the serializer and the deserializer. Wired
clients: the axios-based query clients (`react-query`, `vue-query`,
`svelte-query`, and `solid-query` with `httpClient: 'axios'`), and
`client: 'fetch'` plus `react-query`, `vue-query`, `svelte-query`,
`solid-query`, `swr` and `pinia-colada` under `httpClient: 'fetch'`. With
`fetch`, only the declared success status is converted — error bodies are
...
conversion at all — its return type is unknown to orval and may not even be
a promise, so there's nothing to chain the conversion onto; its request body
is still serialized. The `mcp` client is not transformed. `Date` values in
query and path parameters are still serialized as full ISO datetimes, so
date-only parameters are not
covered.
