...
**Type:** `{ [format: string]: FormatTypeConfig }`
**Default:** `undefined`

Map any OpenAPI string format (e.g. `date`, `date-time`, `time`, `uuid`, `email`) to a custom TypeScript type. Each format can specify its own type, import, Zod schema (transform or codec), and mock expression.

`formatType` takes precedence over `useDates` when both are set. You do not need `useDates: true` when using `formatType`.

#### Config shape

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `type` | `string` | Yes | — | TypeScript type name (e.g. `'Dayjs'`, `'Temporal.PlainDate'`, `'UUID'`) |
...
| `zod.codec` | `string` | No | — | Replace entire Zod schema chain with a custom codec reference |
| `zod.codecImport` | `{ name, importPath, default? }` | No | — | Import for the codec |
...
> `formatType` currently handles **type mapping** — emitting custom TypeScript types, Zod schemas, and mock expressions. Runtime response/request transforms (deserializing date strings in HTTP responses, serializing custom types in request bodies) are planned for a follow-up.
>
> **Current limitations:**
> - `zod.transform` is only applied for `date`, `date-time`, and `time` formats. Other formats (e.g. `uuid`) support type mapping and codec replacement but not `.transform()`.
> - The Effect client emits `S.String` for non-`Date` formatType values — Effect transform support is not yet implemented.

#### Temporal API example
...
This generates `Temporal.Instant` and `Temporal.PlainDate` in model files instead of `string`.
...
#### Zod codec replacement

For full control over the Zod schema, use `zod.codec` to replace the entire validation chain with a custom codec:
        ...
        formatType: {
          'date-time': {
            ...
            zod: {
              ...
              codecImport: { name: 'isoDatetimeCodec', importPath: './zod-codec' },
            },
          },
        },
      },
...
> Formats not listed in `formatType` fall back to `string`. You can configure only the formats you need. When both `formatType` and `useDates` are set, `formatType` entries take precedence for their configured formats; `useDates` still applies to `date` and `date-time` formats that have no `formatType` entry.
