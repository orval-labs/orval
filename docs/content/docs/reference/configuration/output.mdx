...
When [`schemas`](#schemas) is configured, models go to a dedicated directory with per-file output and a barrel `index.ts`. All schemas — domain types, generic wrappers, and bound aliases — live in this single shared directory and are never duplicated per tag:
...
With [`schemas`](#schemas) configured and `tags-split` mode:
...
pets/
└── pets.ts                       ← import { ApiEnvelopeTemplate, PaginatedPetItems, Pet } from '../models'
owners/
└── owners.ts                     ← import { ApiEnvelopeTemplate, PaginatedOwnerItems, Owner } from '../models'
```
