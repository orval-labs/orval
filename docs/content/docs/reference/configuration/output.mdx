...
|--------|------------|--------|-------|
| `angular` | `override.angular.runtimeValidation` | ✅ | Validates JSON body responses via `Schema.parse()`; skips primitive/`void`, `observe: 'events'/'response'`, and custom mutator paths. [`requestBodies`](#requestbodies) also parses JSON request bodies before sending |
