...
|-------|-----------|--------|-----------------|
| Client | [`target`](#target) | `index.ts` ([`indexFiles`](#indexfiles)) | Your HTTP client, e.g. `@angular/*` |
| Schemas | [`schemas.path`](#schemas) | `index.ts` | None, or `zod` with `type: 'zod'` |
...
Use `split` or `tags-split` mode. In `single` and `tags` mode, mock files are written next to the implementation unless [`mock.path`](#mocks-options) is set, and [`mock.inline`](#mocks-options) puts them back into the implementation file.
...
- [`schemas.importPath`](#importpath): client, MSW and Faker files import schema types from it.
- Faker [`importPath`](#faker-generator-type-faker): MSW files import the response factories from it.
- Faker [`schemasPath`](#faker-generator-type-faker): writes the `schemas: true` factories into the Faker group instead of the schemas directory, which keeps the schemas package free of `@faker-js/faker`.
- Faker [`schemasImportPath`](#schemasimportpath): the specifier operation mocks use for those factories, when a relative path won't do.

MSW files re-export the response factories they import, so `get<Op>ResponseMock` is also reachable from the MSW group. The relocated `schemasPath` file isn't re-exported by `index.faker.ts`; publish it as its own entry point if consumers need `get<Schema>Mock`.
...
└── faker/    index.faker.ts   factories, imports @faker-js/faker
    └── schemas/index.faker.ts get<Schema>Mock factories
```

Orval writes the groups and their barrels. Nx project files, `ng-package.json` and `package.json` `exports` maps are up to your project. `samples/angular-app` generates all four groups from one config (`petstoreArtifactGroups`) and has a Node test that loads the MSW barrel without loading Angular.
...
orval warns when either field names an operation the output does not generate,
since the rule is otherwise ignored without a trace:
...
An operation excluded by an input filter or by
[`useDeprecatedOperations: false`](#usedeprecatedoperations) is not generated
...
// Without it, T remains the union of error response bodies.
```

Operations with no declared error responses pass `never` to `ErrorType`, so the mutator can still add transport or unexpected-response errors to its error type.
...
Optional nullable properties are unchanged: they still pick between the value and `null`, because `null` is a value, not an absence. Setting `nonNullable` as well drops that `null`, and those properties are then spread in the same way.
...
Zod output ignores this option for enums that carry `x-enumNames` or `x-enumDescriptions` metadata, which always emit the `z.enum({ ... })` object form. See the [enums guide](/docs/guides/enums#zod).
...
the field is response-only; this applies even under `preserveReadonlyRequestBodies:
'preserve'`, so a preserved `readOnly` `format: date` field keeps going out as a full
...
a schema that is both array- and object-shaped (OAS 3.1 `type: ['array',
'object']`, or a stray sibling `properties` on a `type: array` schema) also
...
- the axios-based query clients — `react-query`, `vue-query`, `svelte-query`
  and `solid-query` with `httpClient: 'axios'`;
- `client: 'fetch'`, plus `react-query`, `vue-query`, `svelte-query`,
  `solid-query`, `swr` and `pinia-colada` under `httpClient: 'fetch'`.
