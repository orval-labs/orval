...
deeper dates as strings while the generated types claim `Date`. A request body
with several media types, several JSON media types, or a blob body generates no
serializer, since there is no single JSON schema to walk; a schema that is both
array- and object-shaped (OAS 3.1 `type: ['array', 'object']`, or a stray sibling
`properties` on a `type: array` schema) also generates no serializer, since the two
shapes can't be walked together on the request side. Only the
axios-based clients are wired (`react-query`, `vue-query`, `svelte-query`,
and `solid-query` with `httpClient: 'axios'`). `Date` values in query and
path parameters are still serialized as full ISO datetimes, so date-only parameters
are not covered.
