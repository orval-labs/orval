...
Custom handler function that replaces the default response shaping and error mapping of every generated handler. When set, each handler binds the tool arguments to a `fetcher` and calls your function with it, the tool call context, and a `toStructuredContent` function that shapes the body for that operation (as is for a plain object, `{ result: body }` for any other response, `undefined` when the tool declares no `outputSchema`). The return value is used as the tool result as is.
