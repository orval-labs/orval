...
Generate branded (nominal) types for fields marked with `x-brand` in your OpenAPI spec:
...
See the [Branded Types](/docs/guides/branded-types) guide for usage details.
...
Automatically invalidate queries on mutation success (Angular Query & React Query only):
