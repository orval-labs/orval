...
<Callout title="Side Effects">
  If added, `useInfinite` and `useSuspenseInfinite` will only create `useInfiniteQuery` and `useSuspenseInfiniteQuery` hooks for endpoints that use this query parameter. 
</Callout>
