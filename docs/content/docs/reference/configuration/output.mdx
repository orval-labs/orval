...
Enabling this for a non-`GET` operation adds a Suspense Query hook alongside
whatever the operation already generates — it does not change whether the
operation is a Query or a Mutation. A `POST` operation that generates a
Mutation hook by default keeps that Mutation hook and additionally gets a
Suspense Query hook.
...
Query hook wins for non-`GET`. This tie-break applies to `useQuery` only —
`useSuspenseQuery`, `useInfinite`, and `useSuspenseInfiniteQuery` never
suppress a Mutation hook; they add a hook of their own shape instead.
