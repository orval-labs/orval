...
The mutator owns parsing, validation, and error handling. Matching the supplied status and media type does not validate the response body. Named and default mutator exports are supported; hook mutators are not. Only explicit error status codes are supported: `default` and ranges such as `4XX` cause generation to fail when this option is enabled.
