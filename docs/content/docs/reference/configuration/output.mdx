...
Configures one or more mock generators. The shorthand `mocks: true` enables both MSW and Faker mock files with default options:
...
Each entry in `mocks.generators` produces its own file (`<filename>.msw.ts`, `<filename>.faker.ts`, ...). Set `mocks: false` (or omit it) to disable mock generation entirely.
