...
route. Empty, absolute, or parent-escaping route values are rejected. Routes
are supported for TypeScript and Zod schemas; enums use `routes.enum` and all
other schemas use `routes.default`. Routes can be combined with `splitByTags`.
They remain incompatible with `operationSchemas`, mocks, and factory methods.
...
Supported by the `fetch` client. Requires `schemas: { type: 'zod' }` and
