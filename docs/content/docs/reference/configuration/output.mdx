...
> **Note:** Only valid when using Angular.
...
By default (without this option) orval already preserves schema-declared
object and array-of-object params so a `paramsSerializer`/`mutator` can
handle them — see the
[Angular guide](/docs/guides/angular#query-parameter-filtering). Use
`paramsFilter` only when you need control the schema cannot express.
