...
object-shaped is converted structurally instead: each property a variant
declares is guarded by its presence on the value (`'key' in value`), which is
also what lets TypeScript narrow the union. A property the variants treat
differently is skipped, while the rest of the union still converts — because
the response direction converts every date format identically, a
`date`-versus-`date-time` disagreement between variants only affects the
request direction. A union that mixes object variants with array or scalar
ones is skipped entirely. `additionalProperties` maps are converted with a loop over
...
place of `properties` drops both the serializer and the deserializer. Only the
axios-based clients are wired (`react-query`, `vue-query`, `svelte-query`, and
`solid-query` with `httpClient: 'axios'`). `Date` values in query and path
parameters are still serialized as full ISO datetimes, so date-only parameters
are not covered.
