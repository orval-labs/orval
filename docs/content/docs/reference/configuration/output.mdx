...
|--------|------------|--------|-------|
| `angular` | `override.angular.runtimeValidation` | ✅ | Validates JSON body responses via `Schema.parse()`, including `observe: 'events'/'response'`; types them as the schema's `zod.output` alias; skips primitive/`void` and custom mutator paths. [`requestBodies`](#requestbodies) also parses JSON request bodies before sending |
