...
Format generated files with the specified formatter. Only one formatter can be used at a time. Use `'none'` to explicitly disable formatting (useful to override a global formatter).
