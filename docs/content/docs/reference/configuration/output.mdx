...
Adjusts how the queryKey is filtered, when `shouldFilterQueryKey` is `true`. Default is `'Boolean'`, which will result
in it ending up beeing
```ts title="endpoints.ts"
.filter(Boolean)
```

One option could be to only make it filter out undefined keys:
```ts title="orval.config.ts"
