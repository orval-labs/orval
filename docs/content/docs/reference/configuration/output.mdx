...
- When `schemas:` is configured (string or `{ type: 'zod' }`) and `client: 'zod'` is set, the schemas directory holds reusable Zod schemas instead of TypeScript types. Set `fileExtension` to disambiguate output if you keep both TS types and reusable Zod schemas in the same directory.
