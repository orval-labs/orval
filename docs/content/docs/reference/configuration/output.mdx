...
When `true`, nullable properties are generated without `faker.helpers.arrayElement([value, null])`. Optional properties may still be omitted via `arrayElement([value, undefined])` unless `required` is also `true`. You can still pass `null` at runtime through the factory's `overrideResponse` argument.
