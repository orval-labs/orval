...
|--------|------------|--------|-------|
| `angular` | `override.angular.runtimeValidation` | ✅ | `throw`: `Schema.parse()`; `both`: `safeParse` + `console.error` + throw. Skips primitive/`void`, `observe: 'events'/'response'`, and custom mutator paths |
| `angular-query` | `override.query.runtimeValidation` | ✅ | `throw`: `Schema.parse()` in RxJS pipeline; `both`: `safeParse` + `console.error` + throw. Skips primitive/`void` and custom mutator paths |
| `fetch` | `override.fetch.runtimeValidation` | ✅ | `throw`: `Schema.parse()`; `both`: `safeParse` + `console.error` + throw for JSON responses |
