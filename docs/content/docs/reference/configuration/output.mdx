...
with multiple success shapes are skipped, and only the axios-based clients
are wired (`react-query`, `vue-query`, `svelte-query`, and `solid-query`
with `httpClient: 'axios'`).
