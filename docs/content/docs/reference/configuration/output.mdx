...
|--------|------------|--------|-------|
| `angular` | `override.angular.runtimeValidation` | ✅ | `throw`: `Schema.parse()`; `both`: `safeParse` + `console.error` + throw. Validates `observe: 'events'/'response'` by cloning the response body. Skips primitive/`void` and custom mutator paths |
