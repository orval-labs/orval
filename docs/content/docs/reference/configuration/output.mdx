...
**Configured mock directories are pruned.** When [`mock.path`](#mock) or a mock generator's own `path` is set, mock files land outside `target` and `schemas`, and that directory is often one you also keep hand-written code in — MSW's `browser.ts` and `server.ts`, fixtures, or a barrel. Only files Orval could have written there are removed, meaning `*.msw.*` and `*.faker.*`. Everything else is left untouched.
...
Keep hand-written files (mutators, transformers, app code) outside the `target` and `schemas` directories for the same reason. A configured mock directory is safe to share with hand-written code, since it is pruned rather than wiped.
