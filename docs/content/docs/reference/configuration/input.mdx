...
When `tags` is set (and `schemas` is not), orval emits only the schemas referenced by the matching operations. Set `includeUnreferencedSchemas: true` to keep **every** `#/components` entry — including schemas referenced by no operation — while still filtering endpoints by `tags`. This works in both `include` and `exclude` mode. It is ignored when `schemas` is set.
