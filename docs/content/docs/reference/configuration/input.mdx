...
The transformer function receives an `OpenApiDocument` and should return a transformed `OpenApiDocument`. You may mutate the input in place or return a new object. You can use the `defineTransformer` helper from `orval` for type inference.
...
If you also specify `schemas`, it takes precedence and the automatic inference is skipped. To filter endpoints by tags while still outputting all schemas, use `schemas: [/.*/]`.
