...
When using Zod schemas, you can enable automatic runtime validation of HTTP
responses. `runtimeValidation` accepts either:

- `true` (shorthand for `{ strategy: 'throw' }`) to parse and throw
- `{ strategy: 'both' }` to log the raw `ZodError` then throw
