...
`{ parse: <ZodSchema>.parse }` for eligible JSON responses in `httpResource`
output when `override.angular.runtimeValidation` is enabled. This validates
responses at runtime before they reach your component.

`runtimeValidation` is disabled by default for Angular output to preserve
backward compatibility. Set it to `true` when you want generated Angular
clients to validate Zod-backed responses at runtime.
...
If you want automatic Zod parsing, enable `runtimeValidation: true`:
