...
2. Set `override.angular.runtimeValidation` to either:
   ...
   - either object form with `requestBodies: true` to also parse JSON request
     bodies before they are sent (see
     [Parsing request bodies before they are sent](#parsing-request-bodies-before-they-are-sent))
...
### Parsing request bodies before they are sent

When you generate Zod schemas, Orval also emits request-body schemas such as
`CreatePetsBody`, plus matching `z.input<>` / `z.output<>` types. By default,
generated `HttpClient` methods only use those types. They send the body as the
caller passed it.

Set `requestBodies: true` to parse JSON request bodies before transport:
...
The generated method then looks like this:

```ts
createPets(createPetsBody: CreatePetsBody, options?: HttpClientObserveOptions) {
  return defer(() => {
    const parsedCreatePetsBody = CreatePetsBody.parse(createPetsBody);
    // The real method branches on `options.observe`; each branch sends the parsed body.
    return this.http.post(url, parsedCreatePetsBody, options);
  });
}
```

An invalid body never reaches the network. The observable errors with the
`ZodError`, so `catchError` and the `error` callback see it like any other
request failure. Parsing happens on subscribe, not when you call the method.

The parameter keeps the schema's input type, and the parsed output is what gets
sent. Defaults and transforms in the schema therefore apply to the payload.

Form data, url-encoded, binary and text bodies are skipped, and so are
operations with a custom `mutator`. See
[`runtimeValidation.requestBodies`](/docs/reference/configuration/output#requestbodies)
for the details.

Without the option, validate before calling the generated client:
...
That is also the way to go when you want field-level feedback in a form:
`.safeParse()` gives you the issues without throwing.
