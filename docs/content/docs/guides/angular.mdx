...
Two things happen automatically — no configuration needed:
...
2. **Schema-declared object / array-of-object params** are passed through
   untouched, so a downstream `paramsSerializer` or `mutator` can handle them.
   This restores parity with the axios and fetch generators, which never
   filtered these values in the first place.
...
With the spec above, the generated client preserves `filters` so that your
`paramsSerializer` (e.g. `qs.stringify`) or `mutator` (e.g. one that flattens
the object into bracketed keys) actually receives it. See
