...
`override.angular.client` to choose which Angular-style output Orval should
generate.
...
Available Angular output modes:

- `httpClient` — generate injectable service classes backed by `HttpClient`
...
Use `override.angular.client` to choose the generated Angular mode:
...
If you omit `override.angular.client`, Orval uses the default Angular
`HttpClient` service-class output.

### Which mode should you choose?

| `override.angular.client` | What Orval generates | Choose it when |
|---|---|---|
| `httpClient` | Injectable service classes backed by `HttpClient` | You want conventional Angular services, imperative request methods, and the broadest compatibility with request options and mutators |
| `httpResource` | Signal-first `httpResource` functions for retrieval-style operations | You want Angular-native signal ergonomics for reads and prefer resource-based data fetching |
...
- if you do not set `override.angular.client`, Orval defaults to `httpClient`
...
Orval emits a shared helper type for these options. The exact default generic
for `TOmitParse` depends on the generated file, but the emitted shape is:
...
JSON response body is surfaced through `response` and response-event flows.
...
`ResourceState<T>` and `toResourceState()` for integrating a resource into your
own state abstractions.
