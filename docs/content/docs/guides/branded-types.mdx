...
**Note:** Brand names are sanitized to be valid TypeScript identifiers. Special characters are removed, ES5 keywords are prefixed with `_`, and names starting with numbers are prefixed with `N`.
