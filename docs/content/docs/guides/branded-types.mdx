...
When using Zod schemas, branded types are automatically generated using Zod's native `.brand()` method:

```ts
export const PetIdSchema = zod.number().brand<'PetId'>();
export const EmailSchema = zod.string().brand<'Email'>();
```
