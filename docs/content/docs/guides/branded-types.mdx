...
Branded types work seamlessly with other Orval type transformations:

### With `useDates`

When `useDates` is enabled, date strings are branded with `Date` as the base type:
...
```ts
// Generated with useDates: true
export type Timestamp = Branded<Date, 'Timestamp'>;
```

### With `useBigInt`

When `useBigInt` is enabled, 64-bit integers are branded with `bigint` as the base type:
...
properties:
  transactionId:
    type: integer
    format: int64
    x-brand: TransactionId
```

```ts
// Generated with useBigInt: true
