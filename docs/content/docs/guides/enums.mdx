...
Annotated `oneOf`/`const` branches keep the union-of-literals shape. Each annotated branch carries its `title`, `description` and `deprecated` in [`.meta()`](https://zod.dev/metadata), so the values stay readable at runtime through `zod.globalRegistry`:
...
| Zod 3       | No registry, so branches keep `.describe(description)` and `title`/`deprecated` are dropped |

Branches with only a `description`, or with no metadata at all, keep their existing `.describe()` output.
