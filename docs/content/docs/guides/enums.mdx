...
## Zod
Metadata generates `z.enum({ ... })` for Zod4 and `z.nativeEnum({ ... })` for Zod 3, while boolean or mixed-type enums remain literal unions and enums without metadata retain their existing output