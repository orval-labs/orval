...
`toStructuredContent` validates the body with the operation's response schema, dropping fields that are not in the spec, and returns it as is when the response is a plain object, as `{ result: body }` for any other response, and `undefined` when the tool declares no `outputSchema`.
