...
By default, each generated handler returns the response body as text and marks HTTP status codes of 400 and above as errors. When the operation's success response is a plain JSON object, the tool also declares an `outputSchema` and the handler passes the parsed body as `structuredContent`. Arrays, primitives, unions, records and empty responses get neither, because MCP requires both to be objects. To take over response shaping and error mapping, or to use the tool call context for logging, authorization, or elicitation, provide a custom handler via `override.mcp.handler`.
...
Implement the handler with the following signature. Its return value is used as the tool result as is. Return `structuredContent` on success only when the body is a plain object: the generated server declares an `outputSchema` for exactly those tools, and MCP rejects arrays and primitives in `structuredContent`.
