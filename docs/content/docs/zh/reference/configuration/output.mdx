...
**已配置的 mock 目录只会被清理。** 设置了 [`mock.path`](#mock) 或某个 mock 生成器自己的 `path` 时，mock 文件会落在 `target` 和 `schemas` 之外，而那个目录里往往也放着你的手写代码——MSW 的 `browser.ts` 和 `server.ts`、fixtures 或 barrel 文件。此时只会删除 Orval 可能写入的文件，即 `*.msw.*` 和 `*.faker.*`，其余文件保持不动。
...
出于同样的原因，请将手写文件（mutator、transformer、应用代码）放在 `target` 和 `schemas` 目录之外。已配置的 mock 目录可以安全地与手写代码共存，因为它只会被清理而不会被清空。
