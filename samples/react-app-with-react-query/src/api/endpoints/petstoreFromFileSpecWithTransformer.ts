...
>(
 params?: ListPetsParams,
    version= 1, options?: { query?:UseInfiniteQueryOptions<AsyncReturnType<typeof listPets>, Error>, request?: SecondParameter<typeof customInstance>}

  ) => {
  const queryKey = getListPetsQueryKey(params,version);
  const {query: queryOptions, request: requestOptions} = options || {}

  const query = useInfiniteQuery<AsyncReturnType<typeof listPets>, Error>(queryKey, ({ pageParam }) => listPets<Data>({ limit: pageParam, ...params },version, requestOptions), {enabled: !!(version), ...queryOptions} )
...
>(
 petId: string,
    version= 1, options?: { query?:UseInfiniteQueryOptions<AsyncReturnType<typeof showPetById>, Error>, request?: SecondParameter<typeof customInstance>}

  ) => {
  const queryKey = getShowPetByIdQueryKey(petId,version);
  const {query: queryOptions, request: requestOptions} = options || {}

  const query = useInfiniteQuery<AsyncReturnType<typeof showPetById>, Error>(queryKey, () => showPetById<Data>(petId,version, requestOptions), {enabled: !!(version && petId), ...queryOptions} )
