...
const customShowPetByIdHandler = http.get('*/v:version/pets/:petId', ({ request, params }) => {
  const accept = request.headers.get('Accept') || 'application/json';
  const petId = params['petId'];
  const mockPet = { id: Number(petId), name: 'Fluffy', tag: 'cat' };
  
  if (accept.includes('xml') || accept.includes('json')) {
    return HttpResponse.json(mockPet, { headers: { 'Content-Type': accept } });
  }
  return new HttpResponse(`Pet ${petId}: Fluffy the cat`, { headers: { 'Content-Type': 'text/plain' } });
});
