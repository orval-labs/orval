...
export const getListPetsUrl = (params: ListPetsParams) => {
  ...
  return stringifiedParams.length > 0
    ? `http://example.com/pets?${stringifiedParams}`
...
export const getListPetsQueryKey = (params?: ListPetsParams) => {
  return ['http:', 'example.com', 'pets', ...(params ? [params] : [])] as const;
};
  ...
  const onSuccess = (
    ...
    if (!options?.skipInvalidation) {
      queryClient.invalidateQueries({
        queryKey: ['http:', 'example.com', 'pets'],
      });
    }
...
export const getShowPetByIdUrl = (petId: string) => {
  return `http://example.com/pets/${petId}`;
};
...
export const getHealthCheckQueryKey = () => {
  return ['http:', 'example.com', 'health'] as const;
};
...
export const getShowPetWithOwnerUrl = (petId: string) => {
  return `http://example.com/pets/${petId}/owner`;
};
...
export const getShowPetWithOwnerQueryKey = (petId: string) => {
  return ['http:', 'example.com', 'pets', petId, 'owner'] as const;
};
