...
export const getShowPetByIdMutationKey = () => ['showPetById'] as const;
...
>(options?: {
  ...
  TContext
> => {
  const mutationKey = getShowPetByIdMutationKey();
  const { mutation: mutationOptions, fetch: fetchOptions } = options
    ? options.mutation &&
      'mutationKey' in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return showPetById(petId, fetchOptions);
  };
...
export type ShowPetByIdMutationVariables = { petId: string };
...
export const useShowPetById = <TError = Error, TContext = unknown>(
  ...
  TContext
> => {
  return useMutation(getShowPetByIdMutationOptions(options), queryClient);
};
