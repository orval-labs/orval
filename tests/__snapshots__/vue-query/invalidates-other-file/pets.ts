...
import { getHealthCheckQueryKey } from './health';
...
>(
  params: MaybeRefOrGetter<ListPetsParams>,
  headers: MaybeRefOrGetter<ListPetsHeaders>,
  options?: {
    query?: Partial<
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof listPets>>> = ({
    ...
    listPets(toValue(params), toValue(headers), { signal, ...fetchOptions });
  ...
  const onSuccess = (
    ...
    if (!options?.skipInvalidation) {
      queryClient.invalidateQueries({
        predicate: (query) =>
          [getListPetsQueryKey(), getHealthCheckQueryKey()].some((key) =>
