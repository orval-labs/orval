...
import { Pet, PetWithTag, Pets } from '../../schemas';
import type {
  CreatePetsBody,
  CreatePetsParams,
  ListPetsParams,
  ...
  > {
    ...
    if (options?.observe === 'response') {
      return this.http
        ...
        .pipe(
          map((response) => response.clone({ body: Pet.parse(response.body) })),
        );
    }
  ...
  > {
    if (options?.observe === 'events') {
      return this.http
        ...
        .pipe(
          map((event) =>
            event instanceof AngularHttpResponse
              ? event.clone({ body: PetWithTag.parse(event.body) })
              : event,
          ),
        );
    }
    ...
    if (options?.observe === 'response') {
      return this.http
        ...
        .pipe(
          map((response) =>
            response.clone({ body: PetWithTag.parse(response.body) }),
          ),
        );
    }
...
export type ListPetsClientResult = NonNullable<PetsOutput>;
export type CreatePetsClientResult = NonNullable<PetOutput>;
export type ShowPetByIdClientResult = NonNullable<PetOutput>;
export type DeletePetByIdClientResult = NonNullable<void>;
export type ShowPetWithOwnerClientResult = NonNullable<PetWithTagOutput>;
