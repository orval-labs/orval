...
import { CreatePetsBody, Pet, PetWithTag, Pets } from './model';
import type {
  CreatePetsParams,
  ListPetsParams,
  PetOutput,
...
export class SwaggerPetstoreService {
  ...
  > {
    return defer(() => {
      const parsedCreatePetsBody = (() => {
        const result = CreatePetsBody.safeParse(createPetsBody);
        if (!result.success) {
          console.error(
            '[orval] createPets request body validation failed',
            result.error,
          );
          throw result.error;
        }
      ...
      if (options?.observe === 'events') {
        return this.http
          .post<PetOutput>(`/pets`, parsedCreatePetsBody, {
            ...(options as Omit<NonNullable<typeof options>, 'observe'>),
            observe: 'events',
            params: filteredParams,
          })
